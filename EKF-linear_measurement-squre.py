...
N_iter = 800 #2*np.pi*L/(dt*r*us)
rx = 0.1; ry = 0.1; rt = 0.01
ww = 0.10; ws = 0.01;
...
while True:
    ...
    if i<N_iter:
        ...
        real_x = X[0][0]*scale + 100
        real_y = X[1][0]*scale + 100
        Xv = np.append(Xv,X,1)
        Px = np.sqrt(P[0][0])*sf
        Py = np.sqrt(P[1][1])*sf
        ...
        if i>200 and i<400:
            GT[1] = (GT[0][0] + dt*r*uw*np.cos(i*dt*r/L*us + np.pi/2), GT[0][1] + dt*r*uw*np.sin(i*dt*r/L*us + np.pi/2))
        if i>400 and i<600:
            GT[1] = (GT[0][0] + dt*r*uw*np.cos(i*dt*r/L*us + np.pi), GT[0][1] + dt*r*uw*np.sin(i*dt*r/L*us+ np.pi))
        if i>600:
            GT[1] = (GT[0][0] + dt*r*uw*np.cos(i*dt*r/L*us + 3*np.pi/2), GT[0][1] + dt*r*uw*np.sin(i*dt*r/L*us + 3*np.pi/2))
        # GT[1] = (GT[0][0] + dt*r*uw*np.cos(i*dt*r/L*us), GT[0][1] + dt*r*uw*np.sin(i*dt*r/L*us))
            
        G = [(GT[0][0]*scale+100, GT[0][1]*scale+100 ),(GT[1][0]*scale+100  , GT[1][1]*scale+100 )]
